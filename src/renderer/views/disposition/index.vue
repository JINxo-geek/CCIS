...
<!--    <el-form-item style="margin-left:200px" label="条形码" prop="cas"> -->
 ...
 <el-button type="primary" @click="outdialogfunc()">信息查询</el-button>
      ...
      <template slot-scope="scope">
        <el-tag
          ...
          disable-transitions>{{scope.row.hascheckout}}</el-tag>
           ...
           <el-table-column sortable prop="outtime" label="注销时间" width="120">
    </el-table-column>
...
for(var i=this.temp.length-1;i>-1;i--)
    ...
    if(this.temp[i].name == this.search.repo)
  {
    ...
    search1() {
      ...
      // 条件筛选
      this.url = localStorage.ip + '/queryCheckout?repo=' + this.search.repo + '&cas=' + this.search.cas + '&barcode=' + this.search.barcode + '&name=' + this.search.name + '&size=' + this.pageSize + '&start=' + this.currentPage + '&uid=' + this.search.uid
      this.axios.get(this.url).then(body => {
        this.tableDataEnd = body.data.data
